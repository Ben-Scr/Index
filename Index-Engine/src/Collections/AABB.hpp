...
namespace Index {
	...
	struct INDEX_API AABB {
		...
		{
			float radians = Index::Radians<float>(degrees);
			Mat2 rotation = Index::Rotation(radians);
		...
		static bool IsAxisAligned(float radians) {
			...
			const float angles[] = {
				...
				Index::HalfPi<float>(),
				Index::Pi<float>(),
				3 * Index::HalfPi<float>()
			};


			float normalizedRadians = std::fmod(radians, Index::TwoPi<float>());
			if (normalizedRadians < 0) {
				normalizedRadians += Index::TwoPi<float>();
			}
			...
			if (std::abs(normalizedRadians - Index::TwoPi<float>()) < tolerance) {
				return true;
			}
			return false;
		}
