...
namespace Index {
	...
	bool Audio::LoadFromFile(const std::string& filepath) {
		if (filepath.empty()) {
			IDX_CORE_WARN_TAG("Audio", "Empty audio file path");
			return false;
		}
		...
		if (result != MA_SUCCESS) {
			IDX_CORE_ERROR_TAG("Audio", "Failed to load audio: {}", filepath);
			return false;
		}
		...
		if (result == MA_SUCCESS && frameCount > 0) {
			...
			if (sampleCount > maxSamples) {
				IDX_CORE_ERROR_TAG("Audio", "Audio file is too large to cache in memory: {}", filepath);
			...
			if (totalFramesRead == 0) {
				IDX_CORE_ERROR_TAG("Audio", "Failed to decode audio frames: {}", filepath);
				ma_decoder_uninit(&decoder);
				Cleanup();
				return false;
			}
