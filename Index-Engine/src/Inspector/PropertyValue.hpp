...
namespace Index {
	...
	struct PropertyValue {
		...
		// <charconv>/<cstdio> through this header. INDEX_API exports them
		// across the engine DLL boundary so the editor can call them.
		INDEX_API std::string ToString() const;
		INDEX_API static PropertyValue FromString(PropertyType type, const std::string& text);
	};
