...
namespace Index {
	...
	std::uint8_t Random::NextByte(std::uint8_t min, std::uint8_t max) {
		if (min > max) { IDX_CORE_WARN_TAG("Random", "min > max, swapping"); std::swap(min, max); }
	...
	double Random::NextDouble(double max) {
		if (max < 0.0) { IDX_CORE_WARN_TAG("Random", "Negative max clamped to 0"); max = 0.0; }
	...
	float Random::NextFloat(float max) {
		if (max < 0.0f) { IDX_CORE_WARN_TAG("Random", "Negative max clamped to 0"); max = 0.0f; }
	...
	int Random::NextInt(int max) {
		if (max < 0) { IDX_CORE_WARN_TAG("Random", "Negative max clamped to 0"); max = 0; }
